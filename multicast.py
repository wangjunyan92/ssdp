...
class SSDPServer():
    def __init__(self):
        ...
        response = 'HTTP/1.1 200 OK\r\nST: %s\r\n\r\n' % SERVICE_NAME
        self.__s.sendto(response.encode(), (SSDP_ADDR, SSDP_PORT))
        # INFO: 使用默认值
        ...
        # INFO: 添加到多播组
        membership = socket.inet_aton(SSDP_ADDR) + socket.inet_aton(local_ip)
        self.__s.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, membership)
    ...
    def start(self):
        while True:
            ...
            conn.handle_request()
        membership = socket.inet_aton(SSDP_ADDR) + socket.inet_aton(self.local_ip)
        self.__s.setsockopt(socket.IPPROTO_IP, socket.IP_DROP_MEMBERSHIP, membership)
        self.__s.close()
